<x-app-layout>
    ...
    @endpush
    @if ($errors->any())
        <div class="mx-5 mb-5 p-4 rounded-lg bg-red-100 border border-red-400 text-red-700 animate-fade-out">
    ...
    <div class="flex items-center justify-end mx-4 my-5">
        <button command='show-modal' commandfor='addDialog'
    ...
    <div class="overflow-hidden rounded-xl m-5 border border-gray-300 shadow-lg bg-white">
        ...
        <table class="min-w-full text-sm text-gray-700">
            ...
            <tbody class="divide-y divide-gray-200">
                @foreach ($data as $task)
                    <tr class="hover:bg-gray-50 transition duration-200">
                        ...
                        <td class="py-3 px-5">
                            @if ($task->status == 'completed')
                                <span
                                    class="px-3 py-1 text-xs font-bold rounded-full bg-green-100 text-green-700 border border-green-400">Completed</span>
                            @elseif($task->status == 'in_progress')
                                <span
                                    class="px-3 py-1 text-xs font-bold rounded-full bg-yellow-100 text-yellow-700 border border-yellow-400">In
                                    Progress</span>
                            @elseif($task->status == 'pending')
                                <span
                                    class="px-3 py-1 text-xs font-bold rounded-full bg-red-100 text-red-700 border border-red-400">Pending</span>
                            @endif
                        </td>
                        <td class="py-3 px-5 text-center">
                            <button command="show-modal" commandfor="dialog({{ $task->id }})"
                                class="bg-blue-500 hover:bg-blue-600 text-white text-sm font-semibold py-2 px-4 rounded-full">
                                Edit
                            </button>
                        </td>
                        <td class="py-3 px-5 text-center">

                            <button command="show-modal" commandfor="deleteDialog({{ $task->id }})"
                                ...
                                <el-dialog-panel
                                    ...
                                    <div class="bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                        <div
                                            ...
                                            <h2 id="dialog-title" class="text-xl font-semibold text-white">Edit Task
                                            </h2>
                                            <button type="button" command="close"
                                                commandfor="dialog({{ $task->id }})" id="closeDialog"
                                                class="text-gray-400 hover:text-white text-2xl">&times;</button>
                                        </div>

                                        <form action="{{ route('tasks.update', ['task' => $task->id]) }}"
                                            method="post" id="editForm" class="flex flex-col space-y-4 px-6">
                                            @csrf
                                            @method('PATCH')
                                            <!-- Task Name -->
                                            <input type="hidden" name="task_id" value="{{ Auth::user()->id }}">
                                            <input type="hidden" name="project_id" value="{{ $task->project_id }}">
                                            <div class="flex flex-col">
                                                <label for="task_name" class="text-white text-lg mb-2">Task Name
                                                    :</label>
                                                <input type="text" name="task_name" id="task_name"
                                                    class="bg-gray-700 text-white rounded-lg w-full p-2 border border-gray-600 
                                            focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                                    placeholder="Enter Task name" value="{{ $task->task_name }}">
                                            </div>

                                            <!-- Task Description -->
                                            <div class="flex flex-col">
                                                <label for="task_description" class="text-white text-lg mb-2">Task
                                                    Description :</label>
                                                <textarea name="task_description" id="task_description" rows="3" placeholder="Enter Task description"
                                                    class="bg-gray-700 text-white rounded-lg w-full min-h-8 p-2 border border-gray-600 
                                            focus:ring-2 focus:ring-blue-500 focus:outline-none">{{ $task->task_description }}</textarea>
                                            </div>
                                            <div class="flex flex-col">
                                                <label for="deadline" class="text-white text-lg mb-2">Start Date
                                                    :</label>
                                                <input type="date" name="start_date" id="start_date" rows="3"
                                                    placeholder="Enter Task description"
                                                    class="bg-gray-700 text-white rounded-lg min-h-8 w-full p-2 border border-gray-600 
                                            focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                                    value="{{ $task->start_date }}"></input>
                                            </div>
                                            <div class="flex flex-col">
                                                <label for="deadline" class="text-white text-lg mb-2">End Date :</label>
                                                <input type="date" name="end_date" id="end_date" rows="3"
                                                    placeholder="Enter Task description"
                                                    class="bg-gray-700 text-white rounded-lg min-h-8 w-full p-2 border border-gray-600 
                                            focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                                    value="{{ $task->end_date }}"></input>
                                            </div>
                                            <div class="flex flex-col">
                                                <label for="task_id" class="text-white text-lg mb-2">Project
                                                    :</label>
                                                <select name="status" id="task_id"
                                                    class="bg-gray-700 text-white rounded-lg w-full p-2 border border-gray-600 
                                        focus:ring-2 focus:ring-blue-500 focus:outline-none">
                                        ...
                                        <div class="flex justify-end mt-10 pt-5 border-t border-gray-600">
                                            <button command="close" commandfor="dialog({{ $task->id }})"
                    ...
                    {{-- Delete Modal --}}
                    <el-dialog>
                        <dialog id="deleteDialog({{ $task->id }})" aria-labelledby="dialog-title"
                            ...
                            <div class="flex items-center justify-center min-h-full p-6">
                                <el-dialog-panel
                                    ...
                                    <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                                        ...
                                        <button type="button" command="close"
                                            commandfor="deleteDialog({{ $task->id }})"
                                    ...
                                    <div class="p-8">
                                        ...
                                        <p class="pb-5">Are you sure you want to delete {{ $task->name }}?
                                        </p>
                                    </div>
                                    <div class="flex justify-end mt-5 p-5 border-t border-gray-600">
                                        <button command="close" commandfor="deleteDialog({{ $task->id }})"
        ...
        <div
            class="mt-6 flex items-center justify-between bg-gray-50 px-5 py-3 border-t border-gray-200 text-gray-600 text-sm">
            <div>
                Showing
    ...
    <el-dialog>
        <dialog id="addDialog" aria-labelledby="dialog-title"
            class="fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent">
            <el-dialog-backdrop
                class="fixed inset-0 bg-gray-900/50 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"></el-dialog-backdrop>
                ...
                <el-dialog-panel
                    ...
                    <div class="bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex justify-between items-center border-b border-gray-700 px-6 py-4 mb-4">
                            ...
                            <button type="button" command="close" commandfor="addDialog" id="closeDialog"
                                class="text-gray-400 hover:text-white text-2xl">&times;</button>
                        </div>

                        <form action="{{ route('tasks.store') }}" method="post" id="editForm"
                            ...
                            <!-- Task Description -->
                            <div class="flex flex-col">
                                <label for="task_description" class="text-white text-lg mb-2">Task Description
                                    :</label>
                                <textarea name="task_description" id="task_description" rows="3" placeholder="Enter Task description"
                                    class="bg-gray-700 text-white rounded-lg w-full min-h-8 p-2 border border-gray-600 
                            ...
                            <div class="flex flex-col">
                                <label for="task_id" class="text-white text-lg mb-2">Project :</label>
                                <select name="project_id" id="task_id"
                                    class="bg-gray-700 text-white rounded-lg w-full p-2 border border-gray-600 
                            focus:ring-2 focus:ring-blue-500 focus:outline-none">
